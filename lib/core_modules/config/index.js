...
function loadConfig() {
  ...
  config.mongodb.dbOptions.user = config.mongodb.dbOptions.user || process.env.MONGODB_USER
  config.mongodb.dbOptions.pass = config.mongodb.dbOptions.pass || process.env.MONGODB_PASS
  // config.mailer.auth.user = config.mailer.auth.user || process.env.MAILER_AUTH_USER
  // config.mailer.auth.pass = config.mailer.auth.pass || process.env.MAILER_AUTH_PASS
  return config;
}
...
Config.prototype.createConfigurations = function(config){
  ...
  if (config && config.mailer && config.mailer.auth) {
    config.mailer.auth.user = config.mailer.auth.user || process.env.MAILER_AUTH_USER
    config.mailer.auth.pass = config.mailer.auth.pass || process.env.MAILER_AUTH_PASS
  }
  if (config && config.mongodb && config.mongodb.dbOptions) {
    config.mongodb.dbOptions.user = config.mongodb.dbOptions.user || process.env.MONGODB_AUTH_USER
    config.mongodb.dbOptions.pass = config.mongodb.dbOptions.pass || process.env.MONGODB_AUTH_PASS
  }
  if (config && config.aws) {
    config.aws.key = config.aws.key || process.env.AWS_ACCESS_KEY
    config.aws.secret = config.aws.secret || process.env.AWS_ACCESS_SECRET
  }
