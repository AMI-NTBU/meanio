...
var crudCheck = require('./../../../../../config/crudcheck')
...
ExpressEngine.prototype.beginBootstrap = function(meanioinstance, database){
  ...
  // httpServer.connection({port: 3000});

  // httpServer.route({ method: 'GET', path: '/', handler: function (request, reply) { return reply('ok'); } });
  // httpServer.start();
...
function finalRouteHandler(req, res, next) {
  ...
  return res.json(500, {error: 'api endpoitn does not exist...'})
}
